...
\def\presentationtitle{Dijkstrův algoritmus}

\input{images/dijkstra.tex}
...
\begin{document}
    ...
    \begin{frame}{Co již známe\dots}
        \begin{itemize}
            \visible<2->{\item Již známe algoritmy BFS a~DFS.}
            \visible<3->{\item Algoritmus BFS nalezne vždy nejkratší cestu do všech \textbf{dosažitelných vrcholů} v~\textbf{neohodnoceném grafu}.}
            \visible<4->{\item Časová i~prostorová složitost činí $\bigO(n+m)$.}
        \end{itemize}
    ...
    \begin{titleframeenv}{Ohodnocené grafy}
        \obrazek[0.5\textheight]{images/ohodnoceny-graf-nahled.tex}
    \end{titleframeenv}
    ...
    \begin{frame}{Ohodnocený graf}
        \begin{defblock}[Ohodnocený graf]
            \markdarkred{Ohodnoceným grafem} rozumíme uspořádanou trojici $(V,E,w)$, přičemž $G=(V,E)$ je libovolný graf a~$\mapping{w}{E}{\R}$ je zobrazení přiřazující každé hraně $e\in E$ její \markdarkred{váhu} $w(e)$.
        \end{defblock}
        \begin{tinfobox}{Poznámka}
            Někdy je výhodnější se odkázat na samotné koncové vrcholy hrany $u,v$. Abychom si usnadnili zápis, budeme psát $w(u,v)$ místo $w(\set{u,v})$ v~případě \textbf{neorientovaného grafu}, resp. místo $w((u,v))$ v~případě \textbf{orientovaného grafu}.
        \end{tinfobox}
    \end{frame}

    \begin{frame}{Ohodnocený graf}{Příklad}
        \obrazek{images/ohodnoceny-graf.tex}
    \end{frame}
    ...
    \begin{frame}{Vzdálenost vrcholů v~ohodnoceném grafu}
        \visible<2->{\begin{defblock}
            ...
            \[d(u,v)=\min\set{\sum_{e\in P}w(e):\text{$P$ je cesta z~$u$ do~$v$}}.\]
            Pokud $v$ není z~$u$ dosažitelný, pak definujeme $d(u,v)=\infty$.
        \end{defblock}}
        \visible<3->{\obrazek[0.32\textheight]{images/ohodnoceny-graf-vzdalenost-vrcholu.tex}}
    \end{frame}

    \subsection{Hledání cest v~ohodnocených grafech}

    \begin{frame}{Hledání cest v~ohodnocených grafech}
        \visible<2->{\obrazek[0.2\textheight]{images/ohodnoceny-graf-nejkratsi-cesta.tex}}
        ...
        \end{itemize}}
        \visible<5->{\obrazek[0.2\textheight]{images/ohodnoceny-graf-nejkratsi-cesta-2.tex}}
    \end{frame}
    ...
    \begin{titleframeenv}{Dijkstrův algoritmus}
        \obrazek[0.5\textheight]{images/dijkstra-nahled.tex}
    \end{titleframeenv}
    ...
    \begin{frame}{Dijkstrův algoritmus}
        \begin{itemize}
            \visible<2->{\item Edsger Dijkstra (1959)}
            ...
            \visible<7->{\begin{timportantframe}
                ...
                \end{center}
            \end{timportantframe}}
            \visible<8->{\item Stejně jako v~případě BFS si budeme uchovávat \markdarkred{stavy} a~\markdarkred{vzdálenosti} vrcholů.}
        \end{itemize}
    \end{frame}
    ...
    \begin{frame}{Postup Dijkstrova algoritmu}
        \begin{enumerate}
            ...
            \visible<5->{\item Vezmeme všechny sousedy $s$ vrcholu $v$ a~pokud přes $v$ vede do některého z~nich kratší cesta, \textbf{aktualizujeme $D(s)$} a~vrchol $s$ označíme jako \textbf{otevřený}.}
            \visible<6->{\item \textbf{Uzavřeme} vrchol $v$, vracíme se ke 3. kroku a~postup opakujeme.}
        \end{enumerate}
    \end{frame}
            ...
            \begin{minipage}{0.9\textwidth}
                \begin{algorithm}[H]
                    ...
                    \ForEach{\textup{vrchol $v\in V$}}{
                        $\operatorname{stav}(v)\gets\textit{nenalezený},D(v)\gets\infty$\\
                    }
    ...
    {$v_4:7,\ v_6:7,\ v_8:10,\ v_9:11,\ v_7:12$}
    {
        \draw[dijtreeedge] (v1)--(v3);
        \draw[dijtreeedge] (v3)--(v2);
    ...
    \begin{frame}{Proč to funguje?}
        \visible<2->{\begin{talertframe}
            ...
            \end{center}
        \end{talertframe}}
        \visible<3->{\obrazek[0.4\textheight]{images/dijkstra-spravnost.tex}}
        \begin{itemize}
    ...
    \begin{frame}{Nezápornost vah}
        \visible<2->{\begin{talertframe}
            \begin{center}
                Všimněte si, že právě díky \textbf{nezápornosti vah} hran grafu na vstupu algoritmus funguje!
            \end{center}
        \end{talertframe}}
        \visible<3->{\begin{tnoticeframe}
            ...
            \end{center}
        \end{tnoticeframe}}
        \visible<4->{\obrazek[0.28\textheight]{images/zaporny-cyklus.tex}}
    \end{frame}
    ...
    \begin{frame}{Nezápornost vah}
        \visible<2->{\begin{talertframe}
            ...
            \end{center}
        \end{talertframe}}
        \visible<3->{\obrazek[0.45\textheight]{images/hamilton.tex}}
    \end{frame}
    ...
    \begin{titleframeenv}{Složitost algoritmu}
        \obrazek[0.5\textheight]{images/slozitost-nahled.tex}
    \end{titleframeenv}
    ...
    \begin{frame}{Časová složitost s~polem}
        \begin{itemize}
            ...
            \[\bigO(\underbrace{n}_\text{inicializace}+\overbrace{n}^{\text{vnější cyklus}}\cdot\overbrace{n}^{\text{minimum}}+\underbrace{m}_\text{sousedé})=\bigO(n^{2}+m).\]}
        \end{itemize}
    \end{frame}
    ...
    \begin{frame}{Shrnutí a~závěrečný rozbor}
        \visible<2->{\begin{tinfoframe}
            ...
            \end{itemize}
        \end{tinfoframe}}
        \visible<3->{Nyní se nabízí dvojice případů:}
        \begin{itemize}
