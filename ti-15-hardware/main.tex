...
\node[font=\bfseries\large] at (-3.4,3.55) {Pohled shora};

% Disk body
\draw[diskcase] (-6.8,-2.8) rectangle (0.0,2.9);

% Inner frame
\draw[thick, rounded corners=5pt] (-6.35,-2.4) rectangle (-0.45,2.45);

% Platter
\draw[platter] (-4.25,0) circle (1.75);
\draw[thick] (-4.25,0) circle (1.15);

% Hub
\draw[hub] (-4.25,0) circle (0.35);
\draw[hub] (-4.25,0) circle (0.10);

% Screw marks around hub
\foreach \ang in {0,60,120,180,240,300}{
    \fill ({-4.25+0.22*cos(\ang)},{0+0.22*sin(\ang)}) circle (0.025);
}

% Pivot and actuator
\node[pivot, minimum size=0.36cm, circle] (pivotTop) at (-1.9,-1.05) {};

\draw[actuator, rounded corners=4pt]
    (-2.45,-2.10) --
    (-1.05,-2.10) --
    (-0.80,-1.45) --
    (-1.45,-0.55) --
...
% Arm and head
\draw[arm] (pivotTop.center) -- (-3.15,-0.25);
\draw[head, rounded corners=1pt] (-3.32,-0.35) rectangle (-3.02,-0.15);

% Secondary arm
\draw[arm, line width=1pt] (-1.95,-0.75) -- (-2.65,-0.35);

% Small electronics block
\draw[thick, fill=gray!18, rounded corners=3pt]
    (-6.15,-2.05) rectangle (-5.15,-1.45);

% Decorative screws
\foreach \p in {(-6.25,2.25),(-0.65,2.25),(-6.25,-2.20),(-0.65,-2.20)}{
    \draw[thick] \p circle (0.06);
}

% Top view labels
\node[labelbox, anchor=east] (labTopPlatter) at (-6.95,1.25) {plotna};
\draw[callout] (labTopPlatter.east) -- (-5.45,0.95);

\node[labelbox, anchor=east] (labTopHead) at (-6.95,-0.95) {čtecí/zapisovací hlava};
\draw[callout] (labTopHead.east) -- (-3.17,-0.25);

\node[labelbox, anchor=west] (labTopArm) at (0.15,3.25) {raménko};
\draw[callout] (labTopArm.west) -- (-2.45,-0.45);

\node[labelbox, anchor=west] (labTopAct) at (0.15,-3.00) {aktuátor};
\draw[callout] (labTopAct.west) -- (-1.35,-1.45);
...
\node[font=\bfseries\large] at (4.35,3.55) {Pohled ze strany};

% Side case
\draw[diskcase] (1.0,-2.8) rectangle (7.7,2.9);

% Side inner frame
\draw[thick, rounded corners=5pt] (1.45,-2.4) rectangle (7.25,2.45);

% Platter stack - side view
\draw[platterline] (2.2,1.20) rectangle (5.55,1.42);
\draw[platterline] (2.2,0.40) rectangle (5.55,0.62);
\draw[platterline] (2.2,-0.40) rectangle (5.55,-0.18);
\draw[platterline] (2.2,-1.20) rectangle (5.55,-0.98);

% Elliptic ends for platters
\foreach \y in {1.31,0.51,-0.29,-1.09}{
    \draw[thick, fill=gray!20] (2.2,\y) ellipse (0.16 and 0.11);
    \draw[thick, fill=gray!20] (5.55,\y) ellipse (0.16 and 0.11);
}

% Spindle / central axis
\draw[thick, fill=gray!45] (3.72,-1.55) rectangle (4.05,1.75);
\draw[thick, fill=gray!60] (3.885,1.75) ellipse (0.165 and 0.07);
\draw[thick, fill=gray!60] (3.885,-1.55) ellipse (0.165 and 0.07);

% Spindle cap
\draw[hub] (3.55,1.85) rectangle (4.22,2.05);
\draw[hub] (3.55,-1.85) rectangle (4.22,-1.65);

% Actuator and pivot from side
\node[pivot, minimum size=0.34cm, circle] (pivotSide) at (6.45,-1.35) {};
...
% Side arms between platters
\draw[arm] (pivotSide.center) -- (5.45,0.50);
\draw[arm] (pivotSide.center) -- (5.45,-0.30);

% Heads
\draw[head, rounded corners=1pt] (5.25,0.38) rectangle (5.55,0.58);
\draw[head, rounded corners=1pt] (5.25,-0.42) rectangle (5.55,-0.22);

% Small gap labels
\node[labelbox, anchor=west] (labSidePlatters) at (7.85,1.25) {více ploten nad sebou};
\draw[callout] (labSidePlatters.west) -- (5.55,1.31);

\node[labelbox, anchor=west] (labSideSpindle) at (7.85,0.10) {vřeteno};
\draw[callout] (labSideSpindle.west) -- (4.05,0.10);

\node[labelbox, anchor=west] (labSideHead) at (7.85,-0.75) {hlavy mezi plotnami};
\draw[callout] (labSideHead.west) -- (5.48,-0.30);

\node[labelbox, anchor=west] (labSideAct) at (7.85,-2.85) {aktuátor};
\draw[callout] (labSideAct.west) -- (6.55,-1.55);
...
\node[
    ...
    HDD ukládá data na rotující magnetické plotny.
    Čtecí/zapisovací hlava se pohybuje nad povrchem plotny a vybírá požadovanou stopu.
};
...
% Arrow from highlighted sector to detail
\draw[callout] (1.55,1.15) -- (3.20,0.65);
...
\node[
    ...
    Plotna je rozdělena na soustředné \textbf{stopy}.
    Každá stopa se dále dělí na \textbf{sektory},
    které tvoří základní oblasti pro ukládání dat.
};
